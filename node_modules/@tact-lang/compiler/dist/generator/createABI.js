...
function createABI(ctx, name) {
    ...
    for (const f of contract.functions.values()) {
        if (f.isGetter) {
            getters.push({
                ...
                returnType: f.returns.kind !== "void"
                    ? (0, resolveABITypeRef_1.createABITypeRefFromTypeRef)(f.returns, f.ast.ref)
                    : null,
            });
        }
    }
    ...
    for (const e of Object.values(errors_1.contractErrors)) {
        errors[e.id] = { message: e.message };
    }
    const codeErrors = (0, resolveErrors_1.getAllErrors)(ctx);
    ...
    const interfaces = [
        ...
        ...(0, getSupportedInterfaces_1.getSupportedInterfaces)(contract, ctx),
    ];
    return {
        name: contract.name,
        types,
