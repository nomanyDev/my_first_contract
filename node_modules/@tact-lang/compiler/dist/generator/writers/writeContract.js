...
function writeInit(t, init, ctx) {
    ctx.fun(ops_1.ops.contractInit(t.name, ctx), () => {
        ...
        ctx.body(() => {
            ...
            // Generate statements
            const returns = (0, resolveFuncTypeUnpack_1.resolveFuncTypeUnpack)(t, (0, id_1.id)("self"), ctx);
    ...
    ctx.fun(ops_1.ops.contractInitChild(t.name, ctx), () => {
        const args = [
            `cell sys'`,
            ...init.args.map((v) => (0, resolveFuncType_1.resolveFuncType)(v.type, ctx) + " " + (0, id_1.id)(v.name)),
        ];
        ...
        ctx.body(() => {
            ...
            const args = t.init.args.length > 0
                ? [
                    "b",
                    "(" +
                        t.init.args.map((a) => (0, id_1.id)(a.name)).join(", ") +
                        ")",
                ].join(", ")
                : "b, null()";
