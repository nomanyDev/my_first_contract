...
function writeRouter(type, kind, ctx) {
    ...
    ctx.inIndent(() => {
        ...
        if (internal) {
            ...
            ctx.inIndent(() => {
                ...
                for (const r of bounceReceivers) {
                    ...
                    const allocation = (0, resolveDescriptors_1.getType)(ctx.ctx, selector.type);
                    if (!allocation)
        ...
        // Text resolvers
        const hasComments = !!type.receivers.find((v) => internal
        ...
        if (hasComments) {
            ...
            ctx.append(`if (op == 0) {`);
            ctx.inIndent(() => {
                if (type.receivers.find((v) => v.selector.kind ===
                    (internal
                        ? "internal-comment"
                        : "external-comment"))) {
                ...
                const fallback = type.receivers.find((v) => v.selector.kind ===
                    (internal
        ...
        // Fallback
        const fallbackReceiver = type.receivers.find((v) => v.selector.kind ===
...
function writeReceiver(self, f, ctx) {
    const selector = f.selector;
    const selfRes = (0, resolveFuncTypeUnpack_1.resolveFuncTypeUnpack)(self, (0, id_1.id)("self"), ctx);
    const selfType = (0, resolveFuncType_1.resolveFuncType)(self, ctx);
    const selfUnpack = `var ${(0, resolveFuncTypeUnpack_1.resolveFuncTypeUnpack)(self, (0, id_1.id)("self"), ctx)} = ${(0, id_1.id)("self")};`;
    // Binary receiver
    ...
    if (selector.kind === "bounce-binary") {
        const args = [
            selfType + " " + (0, id_1.id)("self"),
            (0, resolveFuncType_1.resolveFuncType)(selector.type, ctx, false, selector.bounced) +
                " " +
                (0, id_1.id)(selector.name),
        ];
