...
function writeAccessors(type, origin, ctx) {
    ...
    ctx.fun(ops_1.ops.typeToExternal(type.name, ctx), () => {
        ctx.signature(`(${type.fields.map((v) => (0, resolveFuncTupleType_1.resolveFuncTupleType)(v.type, ctx)).join(", ")}) ${ops_1.ops.typeToExternal(type.name, ctx)}((${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}) v)`);
        ctx.flag("inline");
        ctx.context("type:" + type.name);
        ctx.body(() => {
            ctx.append(`var (${type.fields.map((v) => `v'${v.name}`).join(", ")}) = v; `);
