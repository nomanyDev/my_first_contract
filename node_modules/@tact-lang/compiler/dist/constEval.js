...
exports.evalConstantExpression = void 0;
...
function ensureInt(val, source) {
    ...
    if (minTvmInt <= val && val <= maxTvmInt) {
        ...
        throwErrorConstEval(`integer '${val}' does not fit into TVM Int type`, source);
    }
}
...
function evalUnaryOp(op, operand, source, ctx) {
    ...
    switch (op) {
        ...
        case "!!":
            if (valOperand === null) {
                throwErrorConstEval("non-null value expected but got null", operand.ref);
            }
            return valOperand;
    }
}
...
function evalBinaryOp(op, left, right, source, ctx) {
    const valLeft = evalConstantExpression(left, ctx);
    const valRight = evalConstantExpression(right, ctx);
    switch (op) {
        case "+":
            return ensureInt(ensureInt(valLeft, left.ref) + ensureInt(valRight, right.ref), source);
        case "-":
            return ensureInt(ensureInt(valLeft, left.ref) - ensureInt(valRight, right.ref), source);
        case "*":
            return ensureInt(ensureInt(valLeft, left.ref) * ensureInt(valRight, right.ref), source);
        case "/": {
            ...
            if (r === 0n)
                throwErrorConstEval("divisor expression must be non-zero", right.ref);
        ...
        case "&":
            return (ensureInt(valLeft, left.ref) & ensureInt(valRight, right.ref));
        case "|":
            return (ensureInt(valLeft, left.ref) | ensureInt(valRight, right.ref));
        case "^":
            return (ensureInt(valLeft, left.ref) ^ ensureInt(valRight, right.ref));
        case "<<": {
            ...
            if (0n > valBits || valBits > 256n) {
                throwErrorConstEval(`the number of bits shifted ('${valBits}') must be within [0..256] range`, right.ref);
            }
            try {
        ...
        case "!=":
            ...
            return valLeft !== valRight;
        case "&&":
            return (ensureBoolean(valLeft, left.ref) &&
...
function evalConditional(condition, thenBranch, elseBranch, ctx) {
    // here we rely on the typechecker that both branches have the same type
    const valCond = ensureBoolean(evalConstantExpression(condition, ctx), condition.ref);
    if (valCond) {
...
function evalStructInstance(structTypeId, structFields, ctx) {
    return structFields.reduce((resObj, fieldWithInit) => {
        resObj[fieldWithInit.name] = evalConstantExpression(fieldWithInit.exp, ctx);
        return resObj;
    }, { $tactStruct: structTypeId });
}
function evalFieldAccess(structExpr, fieldId, source, ctx) {
    // special case for contract/trait constant accesses via `self.constant`
    if (structExpr.kind === "id" && structExpr.value == "self") {
        const selfTypeRef = (0, resolveExpression_1.getExpType)(ctx, structExpr);
        if (selfTypeRef.kind == "ref") {
            const contractTypeDescription = (0, resolveDescriptors_1.getType)(ctx, selfTypeRef.name);
            const foundContractConst = contractTypeDescription.constants.find((constId) => constId.name === fieldId);
            if (foundContractConst === undefined) {
                // not a constant, e.g. `self.storageVariable`
                throwNonFatalErrorConstEval(`cannot a evaluate non-constant self field access`, structExpr.ref);
            }
            if (foundContractConst.value !== undefined) {
                return foundContractConst.value;
            }
            else {
                throwErrorConstEval(`cannot evaluate declared contract/trait constant "${fieldId}" as it does not have a body`, source);
            }
        }
    }
    ...
    if (valStruct == null ||
        ...
        throwErrorConstEval(`constant struct expected, but got ${valStruct}`, structExpr.ref);
    }
    if (fieldId in valStruct) {
        ...
        throwErrorConstEval(`struct field ${fieldId} is missing`, structExpr.ref);
    }
}
function evalMethod(methodName, object, args, source, ctx) {
    switch (methodName) {
        case "asComment": {
            ensureMethodArity(0, args, source);
            const comment = ensureString(evalConstantExpression(object, ctx), object.ref);
            return new types_1.CommentValue(comment);
        }
        default:
            throwNonFatalErrorConstEval(`calls of "${methodName}" are not supported at this moment`, source);
    }
}
function evalBuiltins(builtinName, args, source, ctx) {
    switch (builtinName) {
        case "ton": {
            ensureFunArity(1, args, source);
            const tons = ensureString(evalConstantExpression(args[0], ctx), args[0].ref);
            try {
                return ensureInt(BigInt((0, core_1.toNano)(tons).toString(10)), source);
            }
            catch (e) {
        ...
        case "pow": {
            ...
            const valBase = ensureInt(evalConstantExpression(args[0], ctx), args[0].ref);
            const valExp = ensureInt(evalConstantExpression(args[1], ctx), args[1].ref);
            if (valExp < 0n) {
                throwErrorConstEval(`'pow()' builtin called with negative exponent ${valExp}`, source);
            }
            try {
        ...
        case "pow2": {
            ensureFunArity(1, args, source);
            const valExponent = ensureInt(evalConstantExpression(args[0], ctx), args[0].ref);
            if (valExponent < 0n) {
                throwErrorConstEval(`'pow2()' builtin called with negative exponent ${valExponent}`, source);
            }
            try {
        ...
        case "sha256": {
            ensureFunArity(1, args, source);
            const str = ensureString(evalConstantExpression(args[0], ctx), args[0].ref);
            const dataSize = Buffer.from(str).length;
            if (dataSize > 128) {
                throwErrorConstEval(`data is too large for sha256 hash, expected up to 128 bytes, got ${dataSize}`, source);
            }
        ...
        case "cell":
            {
                ...
                try {
                    ...
                    throwErrorConstEval(`invalid base64 encoding for a cell: ${str}`, source);
                }
            }
            break;
        case "address":
            {
                ...
                try {
                    ...
                    if (address.workChain !== 0 && address.workChain !== -1) {
                        throwErrorConstEval(`${str} is invalid address`, source);
                    }
                    if (!(0, features_1.enabledMasterchain)(ctx) && address.workChain !== 0) {
                        throwErrorConstEval(`address ${str} is from masterchain which is not enabled for this contract`, source);
                    }
                    return address;
                }
                catch (_) {
                    throwErrorConstEval(`invalid address encoding: ${str}`, source);
                }
            }
            break;
        case "newAddress": {
            ensureFunArity(2, args, source);
            const wc = ensureInt(evalConstantExpression(args[0], ctx), args[0].ref);
            const addr = Buffer.from(ensureInt(evalConstantExpression(args[1], ctx), args[1].ref).toString(16), "hex");
            if (wc !== 0n && wc !== -1n) {
                throwErrorConstEval(`expected workchain of an address to be equal 0 or -1, received: ${wc}`, source);
            }
            if (!(0, features_1.enabledMasterchain)(ctx) && wc !== 0n) {
                throwErrorConstEval(`${wc}:${addr} address is from masterchain which is not enabled for this contract`, source);
            }
            return new core_1.Address(Number(wc), addr);
        }
        default:
            throwNonFatalErrorConstEval(`unsupported builtin ${builtinName}`, source);
    }
}
function interpretEscapeSequences(stringLiteral) {
...
function evalConstantExpression(ast, ctx) {
    switch (ast.kind) {
        case "id":
            if ((0, resolveDescriptors_1.hasStaticConstant)(ctx, ast.value)) {
                const constant = (0, resolveDescriptors_1.getStaticConstant)(ctx, ast.value);
                if (constant.value !== undefined) {
                    return constant.value;
                }
                else {
                    throwErrorConstEval(`cannot evaluate declared constant "${ast.value}" as it does not have a body`, ast.ref);
                }
            }
        ...
        case "init_of":
            throwNonFatalErrorConstEval("initOf is not supported at this moment", ast.ref);
            break;
        case "lvalue_ref":
        ...
        case "conditional":
            return evalConditional(ast.condition, ast.thenBranch, ast.elseBranch, ctx);
        case "op_new":
