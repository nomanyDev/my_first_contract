...
function getSortedTypes(ctx) {
    const types = Object.values((0, resolveDescriptors_1.getAllTypes)(ctx)).filter((v) => v.kind === "struct" || v.kind === "contract");
    ...
    for (const s of types) {
        if (s.kind === "contract" && s.init) {
            ...
            for (const f of s.init.args) {
                const abiType = (0, resolveABITypeRef_1.createABITypeRefFromTypeRef)(f.type, f.ref);
                ops.push({
                    name: f.name,
                    type: abiType,
