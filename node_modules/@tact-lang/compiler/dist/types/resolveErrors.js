...
function stringId(src) {
    return (0, crypto_1.sha256_sync)(src).readUInt32BE(0);
}
...
function resolveStringsInAST(ast, ctx) {
    (0, ast_1.traverse)(ast, (node) => {
        if (node.kind === "op_static_call" && node.name === "require") {
            ...
            const resolved = (0, constEval_1.evalConstantExpression)(node.args[1], ctx);
...
function resolveErrors(ctx) {
    ...
    for (const f of Object.values((0, resolveDescriptors_1.getAllStaticConstants)(ctx))) {
        ctx = resolveStringsInAST(f.ast, ctx);
    }
    // Process all types
