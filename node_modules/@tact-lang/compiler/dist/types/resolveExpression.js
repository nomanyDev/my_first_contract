...
function resolveStructNew(exp, sctx, ctx) {
    ...
    if (tp.kind !== "struct") {
        (0, errors_1.throwCompilationError)(`Invalid type "${exp.type}" for construction`, exp.ref);
    }
    // Process fields
    ...
    for (const e of exp.args) {
        ...
        if (processed.has(e.name)) {
            (0, errors_1.throwCompilationError)(`Duplicate fields "${e.name}"`, e.ref);
        }
        processed.add(e.name);
        // Check existing
        const f = tp.fields.find((v) => v.name === e.name);
        if (!f) {
            (0, errors_1.throwCompilationError)(`Unknown fields "${e.name}" in type "${tp.name}"`, e.ref);
        }
        // Resolve expression
        ...
        if (!(0, subtyping_1.isAssignable)(expressionType, f.type)) {
            (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(expressionType)}" for fields "${e.name}" with type "${(0, types_1.printTypeRef)(f.type)}" in type "${tp.name}"`, e.ref);
        }
    }
    // Check missing fields
    for (const f of tp.fields) {
        if (f.default === undefined && !processed.has(f.name)) {
            (0, errors_1.throwCompilationError)(`Missing fields "${f.name}" in type "${tp.name}"`, exp.ref);
        }
    }
    // Register result
    return registerExpType(ctx, exp, {
...
function resolveBinaryOp(exp, sctx, ctx) {
    ...
    if (exp.op === "-" ||
        ...
        if (le.kind !== "ref" || le.optional || le.name !== "Int") {
            (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(le)}" for binary operator "${exp.op}"`, exp.ref);
        }
        if (re.kind !== "ref" || re.optional || re.name !== "Int") {
            (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(re)}" for binary operator "${exp.op}"`, exp.ref);
        }
        ...
        if (le.kind !== "null" && re.kind !== "null") {
            ...
            if (l.kind === "map" && r.kind === "map") {
                if (l.key !== r.key ||
                    ...
                    l.keyAs !== r.keyAs ||
                    l.valueAs !== r.valueAs) {
                    (0, errors_1.throwCompilationError)(`Incompatible types "${(0, types_1.printTypeRef)(le)}" and "${(0, types_1.printTypeRef)(re)}" for binary operator "${exp.op}"`, exp.ref);
                }
            }
            else {
                if (l.kind === "ref_bounced" || r.kind === "ref_bounced") {
                    (0, errors_1.throwCompilationError)("Bounced types are not supported in binary operators", exp.ref);
                }
                if (l.kind == "void" || r.kind == "void") {
                    (0, errors_1.throwCompilationError)(`Expressions of "<void>" type cannot be used for (non)equality operator "${exp.op}"`, exp.ref);
                }
                ...
                if (r.name !== "Int" &&
                    ...
                    r.name !== "Slice" &&
                    r.name !== "String") {
                    (0, errors_1.throwCompilationError)(`Invalid type "${r.name}" for binary operator "${exp.op}"`, exp.ref);
                }
            }
        }
        resolved = { kind: "ref", name: "Bool", optional: false };
...
function resolveUnaryOp(exp, sctx, ctx) {
    ...
    if (exp.op === "-" || exp.op === "+" || exp.op === "~") {
        if (resolvedType.kind !== "ref" ||
            ...
            (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(resolvedType)}" for unary operator "${exp.op}"`, exp.ref);
        }
    }
    else if (exp.op === "!") {
        if (resolvedType.kind !== "ref" ||
        ...
        if (resolvedType.kind !== "ref" || !resolvedType.optional) {
            (0, errors_1.throwCompilationError)(`Type "${(0, types_1.printTypeRef)(resolvedType)}" is not optional`, exp.ref);
        }
        resolvedType = {
            kind: "ref",
        ...
        (0, errors_1.throwCompilationError)("Unknown operator " + exp.op, exp.ref);
    }
    // Register result
...
function resolveField(exp, sctx, ctx) {
    ...
    if (src === null ||
        ...
        (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(src)}" for field access`, exp.ref);
    }
    ...
    if (sctx.requiredFields.length > 0 &&
        ...
        if (sctx.requiredFields.find((v) => v === exp.name)) {
            (0, errors_1.throwCompilationError)(`Field "${exp.name}" is not initialized`, exp.ref);
        }
    }
    // Find field
    let fields;
    ...
    if (!field && !cst) {
        if (src.kind === "ref_bounced") {
            (0, errors_1.throwCompilationError)(`Type bounced<"${src.name}"> does not have a field named "${exp.name}"`, exp.ref);
        }
        else {
            (0, errors_1.throwCompilationError)(`Type "${src.name}" does not have a field named "${exp.name}"`, exp.ref);
        }
    }
...
function resolveStaticCall(exp, sctx, ctx) {
    ...
    if (global_1.GlobalFunctions.has(exp.name)) {
        const f = global_1.GlobalFunctions.get(exp.name);
        // Resolve arguments
        for (const e of exp.args) {
    ...
    if (!(0, resolveDescriptors_1.hasStaticFunction)(ctx, exp.name)) {
        (0, errors_1.throwCompilationError)(`Static function "${exp.name}" does not exist`, exp.ref);
    }
    // Get static function
    ...
    if (f.args.length !== exp.args.length) {
        (0, errors_1.throwCompilationError)(`Function "${exp.name}" expects ${f.args.length} arguments, got ${exp.args.length}`, exp.ref);
    }
    for (let i = 0; i < f.args.length; i++) {
        ...
        if (!(0, subtyping_1.isAssignable)(t, a.type)) {
            (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(t)}" for argument "${a.name}"`, e.ref);
        }
    }
    // Resolve return type
...
function resolveCall(exp, sctx, ctx) {
    ...
    if (exp.src.kind === "id" &&
        ...
        (0, errors_1.throwCompilationError)("Cannot access self before init", exp.ref);
    }
    // Resolve args
    for (const e of exp.args) {
        ctx = resolveExpression(e, sctx, ctx);
    ...
    const src = getExpType(ctx, exp.src);
    if (src === null) {
        (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(src)}" for function call`, exp.ref);
    }
    // Handle ref
    if (src.kind === "ref") {
        if (src.optional) {
        ...
        // Check struct ABI
        if (srcT.kind === "struct") {
            if (struct_1.StructFunctions.has(exp.name)) {
                const abi = struct_1.StructFunctions.get(exp.name);
        ...
        if (!f) {
            (0, errors_1.throwCompilationError)(`Type "${src.name}" does not have a function named "${exp.name}"`, exp.ref);
        }
        // Check arguments
        if (f.args.length !== exp.args.length) {
            (0, errors_1.throwCompilationError)(`Function "${exp.name}" expects ${f.args.length} arguments, got ${exp.args.length}`, exp.ref);
    ...
    // Handle map
    if (src.kind === "map") {
        if (!map_1.MapFunctions.has(exp.name)) {
            (0, errors_1.throwCompilationError)(`Map function "${exp.name}" not found`, exp.ref);
        }
        const abf = map_1.MapFunctions.get(exp.name);
        const resolved = abf.resolve(ctx, [src, ...exp.args.map((v) => getExpType(ctx, v))], exp.ref);
        return registerExpType(ctx, exp, resolved);
    }
    if (src.kind === "ref_bounced") {
        (0, errors_1.throwCompilationError)(`Cannot call function on bounced value`, exp.ref);
    }
    (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(src)}" for function call`, exp.ref);
}
function resolveInitOf(ast, sctx, ctx) {
    ...
    if (type.kind !== "contract") {
        (0, errors_1.throwCompilationError)(`Type "${ast.name}" is not a contract`, ast.ref);
    }
    if (!type.init) {
        (0, errors_1.throwCompilationError)(`Contract "${ast.name}" does not have an init function`, ast.ref);
    }
    // Resolve args
    ...
    if (type.init.args.length !== ast.args.length) {
        (0, errors_1.throwCompilationError)(`Init function of "${type.name}" expects ${type.init.args.length} arguments, got ${ast.args.length}`, ast.ref);
    }
...
function resolveConditional(ast, sctx, ctx) {
    ...
    if (conditionType.kind !== "ref" ||
        ...
        (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(conditionType)}" for ternary condition`, ast.condition.ref);
    }
    ...
    if (resultType) {
        if (resultType.kind == "void") {
            (0, errors_1.throwCompilationError)(`Expressions of "<void>" type cannot be used for conditional expression`, ast.ref);
        }
        return registerExpType(ctx, ast, resultType);
    }
    (0, errors_1.throwCompilationError)(`Non-matching types "${(0, types_1.printTypeRef)(thenType)}" and "${(0, types_1.printTypeRef)(elseType)}" for ternary branches`, ast.elseBranch.ref);
}
...
function resolveLValueRef(path, sctx, ctx) {
    ...
    if (!t) {
        (0, errors_1.throwCompilationError)(`Variable "${paths[0].name}" not found`, paths[0].ref);
    }
    ctx = registerExpType(ctx, paths[0], t);
    // Paths
    for (let i = 1; i < paths.length; i++) {
        if (t.kind !== "ref" || t.optional) {
            (0, errors_1.throwCompilationError)(`Invalid type "${(0, types_1.printTypeRef)(t)}" for field access`, path[i].ref);
        }
        const srcT = (0, resolveDescriptors_1.getType)(ctx, t.name);
        const ex = srcT.fields.find((v) => v.name === paths[i].name);
        if (!ex) {
            (0, errors_1.throwCompilationError)("Field " + paths[i].name + " not found in type " + srcT.name, path[i].ref);
        }
    ...
    if (exp.kind === "id") {
        ...
        if (!v) {
            if (!(0, resolveDescriptors_1.hasStaticConstant)(ctx, exp.value)) {
                if (exp.value === "_") {
                    (0, errors_1.throwCompilationError)("Wildcard variable name '_' cannot be accessed", exp.ref);
                }
                // Handle static struct method calls
                try {
                    const t = (0, resolveDescriptors_1.getType)(ctx, exp.value);
                    if (t.kind === "struct") {
                        return registerExpType(ctx, exp, {
                            kind: "ref",
                            name: t.name,
                ...
                (0, errors_1.throwCompilationError)("Unable to resolve id " + exp.value, exp.ref);
            }
            else {
