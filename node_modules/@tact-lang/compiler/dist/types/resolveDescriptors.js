...
exports.resolvePartialFields = exports.getAllStaticConstants = exports.getAllStaticFunctions = exports.hasStaticConstant = exports.getStaticConstant = exports.hasStaticFunction = exports.getStaticFunction = exports.getContracts = exports.getAllTypes = exports.getType = exports.resolveDescriptors = exports.resolveTypeRef = exports.toBounced = void 0;
...
function verifyMapType(key, keyAs, value, valueAs, ref) {
    if (!keyAs && !valueAs) {
        return;
    }
    // keyAs
    if (keyAs) {
        if (key === "Int") {
            if (![
                ...
                (0, errors_1.throwCompilationError)("Invalid key type for map", ref);
            }
        }
        else {
            (0, errors_1.throwCompilationError)("Invalid key type for map", ref);
    ...
    if (valueAs) {
        if (value === "Int") {
            if (![
                ...
                (0, errors_1.throwCompilationError)("Invalid value type for map", ref);
            }
        }
        else {
            (0, errors_1.throwCompilationError)("Invalid value type for map", ref);
...
function buildTypeRef(src, types) {
    if (src.kind === "type_ref_simple") {
        if (!types.has(src.name)) {
            (0, errors_1.throwCompilationError)("Type " + src.name + " not found", src.ref);
        }
        return {
            kind: "ref",
            name: src.name,
    ...
    if (src.kind === "type_ref_map") {
        if (!types.has(src.key)) {
            (0, errors_1.throwCompilationError)("Type " + src.key + " not found", src.ref);
        }
        if (!types.has(src.value)) {
            (0, errors_1.throwCompilationError)("Type " + src.value + " not found", src.ref);
        }
        return {
            kind: "map",
            key: src.key,
...
function resolveDescriptors(ctx) {
    ...
    for (const a of ast.types) {
        if (types.has(a.name)) {
            (0, errors_1.throwCompilationError)(`Type "${a.name}" already exists`, a.ref);
        }
    ...
    function buildFieldDescription(src, index) {
        ...
        if ((0, isRuntimeType_1.isRuntimeType)(tr)) {
            (0, errors_1.throwCompilationError)((0, types_1.printTypeRef)(tr) +
                " is a runtime only type and can't be used as field", src.ref);
        }
        const d = src.init ? (0, constEval_1.evalConstantExpression)(src.init, ctx) : undefined;
        // Resolve abi type
    ...
    function buildConstantDescription(src) {
        const tr = buildTypeRef(src.type, types);
        const d = src.value
            ? (0, constEval_1.evalConstantExpression)(src.value, ctx)
            : undefined;
    ...
    for (const a of ast.types) {
            ...
            for (const f of a.declarations) {
                if (f.kind === "def_field") {
                    if (types.get(a.name).fields.find((v) => v.name === f.name)) {
                        (0, errors_1.throwCompilationError)(`Field "${f.name}" already exists`, f.ref);
                    }
                    if (types
                        .get(a.name)
                        .constants.find((v) => v.name === f.name)) {
                        (0, errors_1.throwCompilationError)(`Constant "${f.name}" already exists`, f.ref);
                    }
                    types
                        .get(a.name)
                        .fields.push(buildFieldDescription(f, types.get(a.name).fields.length));
                }
                    ...
                    if (f.attributes.find((v) => v.type !== "overrides")) {
                        (0, errors_1.throwCompilationError)(`Constant can be only overridden`, f.ref);
                    }
                    types
                        .get(a.name)
        ...
        if (a.kind === "def_struct") {
            ...
            if (a.fields.length === 0 && !a.message) {
                (0, errors_1.throwCompilationError)(`Struct "${a.name}" must have at least one field`, a.ref);
            }
        }
        // Trait
        if (a.kind === "def_trait") {
            for (const f of a.declarations) {
                if (f.kind === "def_field") {
                    ...
                    if (f.as) {
                        (0, errors_1.throwCompilationError)(`Trait field cannot have serialization specifier`, f.ref);
                    }
                    types
                        .get(a.name)
                        .fields.push(buildFieldDescription(f, types.get(a.name).fields.length));
                    ...
                    if (f.attributes.find((v) => v.type === "overrides")) {
                        (0, errors_1.throwCompilationError)(`Trait constant cannot be overridden`, f.ref);
                    }
    ...
    function resolveFunctionDescriptor(optSelf, a, origin) {
        ...
        if (a.kind === "def_native_function") {
            if (isGetter) {
                (0, errors_1.throwCompilationError)("Native functions cannot be getters", isGetter.ref);
            }
            if (self) {
                (0, errors_1.throwCompilationError)("Native functions cannot be defined within a contract", a.ref);
            }
            if (isVirtual) {
                (0, errors_1.throwCompilationError)("Native functions cannot be virtual", isVirtual.ref);
            }
            if (isOverrides) {
                (0, errors_1.throwCompilationError)("Native functions cannot be overrides", isOverrides.ref);
            }
        }
        // Check virtual and overrides
        if (isVirtual && isExtends) {
            (0, errors_1.throwCompilationError)("Extend functions cannot be virtual", isVirtual.ref);
        }
        if (isOverrides && isExtends) {
            (0, errors_1.throwCompilationError)("Extend functions cannot be overrides", isOverrides.ref);
        }
        if (isAbstract && isExtends) {
            (0, errors_1.throwCompilationError)("Extend functions cannot be abstract", isAbstract.ref);
        }
        if (!self && isVirtual) {
            (0, errors_1.throwCompilationError)("Virtual functions must be defined within a contract or a trait", isVirtual.ref);
        }
        if (!self && isOverrides) {
            (0, errors_1.throwCompilationError)("Overrides functions must be defined within a contract or a trait", isOverrides.ref);
        }
        if (!self && isAbstract) {
            (0, errors_1.throwCompilationError)("Abstract functions must be defined within a trait", isAbstract.ref);
        }
        if (isVirtual && isAbstract) {
            (0, errors_1.throwCompilationError)("Abstract functions cannot be virtual", isAbstract.ref);
        }
        if (isVirtual && isOverrides) {
            (0, errors_1.throwCompilationError)("Overrides functions cannot be virtual", isOverrides.ref);
        }
        if (isAbstract && isOverrides) {
            (0, errors_1.throwCompilationError)("Overrides functions cannot be abstract", isOverrides.ref);
        }
        // Check virtual
        if (isVirtual) {
            const t = types.get(self);
            if (t.kind !== "trait") {
                (0, errors_1.throwCompilationError)("Virtual functions must be defined within a trait", isVirtual.ref);
            }
        }
        // Check abstract
        if (isAbstract) {
        ...
        if (isOverrides) {
            const t = types.get(self);
            if (t.kind !== "contract") {
                (0, errors_1.throwCompilationError)("Overrides functions must be defined within a contract", isOverrides.ref);
            }
        }
        // Check for common
        if (a.kind === "def_function") {
            if (isGetter && !self) {
                (0, errors_1.throwCompilationError)("Getters must be defined within a contract", isGetter.ref);
            }
        }
        // Check for getter
        if (isInline && isGetter) {
            (0, errors_1.throwCompilationError)("Getters cannot be inline", isInline.ref);
        }
        // Validate mutating
        if (isExtends) {
            // Validate arguments
            if (self) {
                (0, errors_1.throwCompilationError)("Extend functions cannot be defined within a contract", isExtends.ref);
            }
            if (args.length === 0) {
                (0, errors_1.throwCompilationError)("Extend functions must have at least one argument", isExtends.ref);
            }
            if (args[0].name !== "self") {
                (0, errors_1.throwCompilationError)('Extend function must have first argument named "self"', args[0].ref);
            }
            if (args[0].type.kind !== "ref") {
                (0, errors_1.throwCompilationError)("Extend functions must have a reference type as the first argument", args[0].ref);
            }
            if (args[0].type.optional) {
                (0, errors_1.throwCompilationError)("Extend functions must have a non-optional type as the first argument", args[0].ref);
            }
            if (!types.has(args[0].type.name)) {
                (0, errors_1.throwCompilationError)("Type " + args[0].type.name + " not found", args[0].ref);
            }
        ...
        if (isMutating && !isExtends) {
            (0, errors_1.throwCompilationError)("Mutating functions must be extend functions", isMutating.ref);
        }
        // Check argument names
        const exNames = new Set();
        for (const arg of args) {
            if (arg.name === "self") {
                (0, errors_1.throwCompilationError)('Argument name "self" is reserved', arg.ref);
            }
            if (exNames.has(arg.name)) {
                (0, errors_1.throwCompilationError)('Argument name "' + arg.name + '" is already used', arg.ref);
            }
        ...
        if (isGetter) {
            for (const arg of args) {
                if ((0, isRuntimeType_1.isRuntimeType)(arg.type)) {
                    (0, errors_1.throwCompilationError)((0, types_1.printTypeRef)(arg.type) +
                        " is a runtime-only type and can't be used as a getter argument", arg.ref);
                }
            }
            if ((0, isRuntimeType_1.isRuntimeType)(returns)) {
                (0, errors_1.throwCompilationError)((0, types_1.printTypeRef)(returns) +
                    " is a runtime-only type and can't be used as getter return type", a.ref);
            }
        }
        // Register function
        return {
            ...
            returns,
            ast: a,
            isMutating: !!isMutating || !!optSelf /* && !isGetter */,
            isGetter: !!isGetter,
            isVirtual: !!isVirtual,
        ...
        for (const a of args) {
            if ((0, isRuntimeType_1.isRuntimeType)(a.type)) {
                (0, errors_1.throwCompilationError)((0, types_1.printTypeRef)(a.type) +
                    " is a runtime-only type and can't be used as a init function argument", a.ref);
            }
        }
        return {
            args,
            ...
            for (const d of a.declarations) {
                if (d.kind === "def_function") {
                    ...
                    if (s.functions.has(f.name)) {
                        (0, errors_1.throwCompilationError)(`Function "${f.name}" already exists in type "${s.name}"`, s.ast.ref);
                    }
                    s.functions.set(f.name, f);
                }
                if (d.kind === "def_init_function") {
                    if (s.init) {
                        (0, errors_1.throwCompilationError)("Init function already exists", d.ref);
                    }
                ...
                if (d.kind === "def_receive") {
                    ...
                    if (d.selector.kind.startsWith("external-") &&
                        ...
                        (0, errors_1.throwCompilationError)("External functions are not enabled", d.ref);
                    }
                    if (d.selector.kind === "internal-simple" ||
                        ...
                        if (arg.type.kind !== "type_ref_simple") {
                            (0, errors_1.throwCompilationError)("Receive function can only accept message", d.ref);
                        }
                        if (arg.type.optional) {
                            (0, errors_1.throwCompilationError)("Receive function cannot have optional argument", d.ref);
                        }
                        // Check resolved argument type
                        const t = types.get(arg.type.name);
                        if (!t) {
                            (0, errors_1.throwCompilationError)("Type " + arg.type.name + " not found", d.ref);
                        }
                        // Raw receiver
                        if (t.kind === "primitive") {
                            if (t.name === "Slice") {
                                ...
                                if (s.receivers.find((v) => v.selector.kind ===
                                    (internal
                                        ? "internal-fallback"
                                        : "external-fallback"))) {
                                    (0, errors_1.throwCompilationError)(`Fallback receive function already exists`, d.ref);
                                }
                                // Persist receiver
                                s.receivers.push({
                                    selector: {
                                ...
                                if (s.receivers.find((v) => v.selector.kind ===
                                    ...
                                    (0, errors_1.throwCompilationError)("Comment fallback receive function already exists", d.ref);
                                }
                                // Persist receiver
                                s.receivers.push({
                                    selector: {
                                ...
                                (0, errors_1.throwCompilationError)("Receive function can only accept message, Slice or String", d.ref);
                            }
                        }
                        else {
                            // Check type
                            ...
                            if (s.receivers.find((v) => v.selector.kind ===
                                (internal
                                    ...
                                    : "external-binary") &&
                                v.selector.name === n)) {
                                (0, errors_1.throwCompilationError)(`Receive function for "${arg.type.name}" already exists`, d.ref);
                            }
                            // Persist receiver
                            s.receivers.push({
                                selector: {
                    ...
                    else if (d.selector.kind === "internal-comment" ||
                        d.selector.kind === "external-comment") {
                        const internal = d.selector.kind === "internal-comment";
                        if (d.selector.comment.value === "") {
                            (0, errors_1.throwCompilationError)("To use empty comment receiver, just remove argument instead of passing empty string", d.ref);
                        }
                        ...
                        if (s.receivers.find((v) => v.selector.kind ===
                            ...
                            (0, errors_1.throwCompilationError)(`Receive function for "${c}" already exists`, d.ref);
                        }
                        s.receivers.push({
                            selector: {
                                kind: internal
                        ...
                        if (s.receivers.find((v) => v.selector.kind ===
                            (internal
                                ? "internal-empty"
                                : "external-empty"))) {
                            (0, errors_1.throwCompilationError)("Empty receive function already exists", d.ref);
                        }
                        s.receivers.push({
                            selector: {
                                kind: internal
                        ...
                        if (arg.type.kind === "type_ref_simple") {
                            if (arg.type.optional) {
                                (0, errors_1.throwCompilationError)("Bounce receive function cannot have optional argument", d.ref);
                            }
                            if (arg.type.name === "Slice") {
                                if (s.receivers.find((v) => v.selector.kind ===
                                    "bounce-fallback")) {
                                    (0, errors_1.throwCompilationError)(`Fallback bounce receive function already exists`, d.ref);
                                }
                                s.receivers.push({
                                    selector: {
                                ...
                                if (type.ast.kind !== "def_struct" ||
                                    !type.ast.message) {
                                    (0, errors_1.throwCompilationError)("Bounce receive function can only accept bounced message, message or Slice", d.ref);
                                }
                                if (type.fields.length !==
                                    type.partialFieldCount) {
                                    (0, errors_1.throwCompilationError)("This message is too big for bounce receiver, you need to wrap it to a bounced<" +
                                        arg.type.name +
                                        ">.", d.ref);
                                }
                                if (s.receivers.find((v) => v.selector.kind ===
                                    ...
                                    (0, errors_1.throwCompilationError)(`Bounce receive function for "${arg.type.name}" already exists`, d.ref);
                                }
                                s.receivers.push({
                                    selector: {
                                        kind: "bounce-binary",
                            ...
                            const t = types.get(arg.type.name);
                            if (t.kind !== "struct") {
                                (0, errors_1.throwCompilationError)("Bounce receive function can only accept bounced<T> struct types", d.ref);
                            }
                            if (t.ast.kind !== "def_struct") {
                                (0, errors_1.throwCompilationError)("Bounce receive function can only accept bounced<T> struct types", d.ref);
                            }
                            ...
                            if (s.receivers.find((v) => v.selector.kind === "bounce-binary" &&
                                v.selector.type === t.name)) {
                                (0, errors_1.throwCompilationError)(`Bounce receive function for "${t.name}" already exists`, d.ref);
                            }
                            if (t.fields.length === t.partialFieldCount) {
                                (0, errors_1.throwCompilationError)("This message is small enough for bounce receiver, you need to remove bounced modifier.", d.ref);
                            }
                            s.receivers.push({
                                selector: {
                                    kind: "bounce-binary",
                            ...
                            (0, errors_1.throwCompilationError)("Bounce receive function can only accept bounced<T> struct args or Slice", d.ref);
                        }
                    }
                    else {
                        (0, errors_1.throwCompilationError)("Invalid receive function selector", d.ref);
                    }
                }
            }
        }
    ...
    for (const t of types.values()) {
        if (t.ast.kind === "def_trait" || t.ast.kind === "def_contract") {
            ...
            function visit(name) {
                ...
                if (!tt) {
                    (0, errors_1.throwCompilationError)("Trait " + name + " not found", t.ast.ref);
                }
                visited.add(name);
                traits.push(tt);
                if (tt.ast.kind === "def_trait") {
                    ...
                    (0, errors_1.throwCompilationError)("Type " + name + " is not a trait", t.ast.ref);
                }
            }
            visit("BaseTrait");
    ...
    for (const t of types.values()) {
        for (const tr of t.traits) {
            ...
            if (!types.has(tr.name)) {
                (0, errors_1.throwCompilationError)("Trait " + tr.name + " not found", t.ast.ref);
            }
            if (types.get(tr.name).kind !== "trait") {
                (0, errors_1.throwCompilationError)("Type " + tr.name + " is not a trait", t.ast.ref);
            }
            ...
            for (const f of ttr.fields) {
                ...
                if (!ex) {
                    (0, errors_1.throwCompilationError)(`Trait "${tr.name}" requires field "${f.name}"`, t.ast.ref);
                }
                // Check type
                if (!(0, types_1.typeRefEquals)(f.type, ex.type)) {
                    (0, errors_1.throwCompilationError)(`Trait "${tr.name}" requires field "${f.name}" of type "${(0, types_1.printTypeRef)(f.type)}"`, t.ast.ref);
                }
            }
        }
    }
    ...
    function copyTraits(t) {
        for (const tr of t.traits) {
            ...
            for (const f of tr.functions.values()) {
                ...
                if (!ex && f.isAbstract) {
                    (0, errors_1.throwCompilationError)(`Trait "${tr.name}" requires function "${f.name}"`, t.ast.ref);
                }
                // Check overrides
                if (ex && ex.isOverrides) {
                    if (f.isGetter) {
                        (0, errors_1.throwCompilationError)(`Overridden function "${f.name}" can not be a getter`, ex.ast.ref);
                    }
                    if (f.isMutating !== ex.isMutating) {
                        (0, errors_1.throwCompilationError)(`Overridden function "${f.name}" should have same mutability`, ex.ast.ref);
                    }
                    if (!(0, types_1.typeRefEquals)(f.returns, ex.returns)) {
                        (0, errors_1.throwCompilationError)(`Overridden function "${f.name}" should have same return type`, ex.ast.ref);
                    }
                    if (f.args.length !== ex.args.length) {
                        (0, errors_1.throwCompilationError)(`Overridden function "${f.name}" should have same number of arguments`, ex.ast.ref);
                    }
                    for (let i = 0; i < f.args.length; i++) {
                        const a = ex.args[i];
                        const b = f.args[i];
                        if (!(0, types_1.typeRefEquals)(a.type, b.type)) {
                            (0, errors_1.throwCompilationError)(`Overridden function "${f.name}" should have same argument types`, ex.ast.ref);
                        }
                    }
                ...
                // Check duplicates
                if (ex) {
                    (0, errors_1.throwCompilationError)(`Function "${f.name}" already exist in "${t.name}"`, t.ast.ref);
                }
                // Register function
            ...
            for (const f of tr.constants) {
                ...
                if (!ex &&
                    ...
                    (0, errors_1.throwCompilationError)(`Trait "${tr.name}" requires constant "${f.name}"`, t.ast.ref);
                }
                // Check overrides
                if (ex &&
                    ex.ast.attributes.find((v) => v.type === "overrides")) {
                    if (!(0, types_1.typeRefEquals)(f.type, ex.type)) {
                        (0, errors_1.throwCompilationError)(`Overridden constant "${f.name}" should have same type`, ex.ast.ref);
                    }
                    continue;
                }
                // Check duplicates
                if (ex) {
                    (0, errors_1.throwCompilationError)(`Constant "${f.name}" already exist in "${t.name}"`, t.ast.ref);
                }
                // Register constant
                t.constants.push({
                    ...f,
            ...
            for (const f of tr.receivers) {
                ...
                if (t.receivers.find((v) => sameReceiver(v.selector, f.selector))) {
                    (0, errors_1.throwCompilationError)(`Receive function for "${f.selector}" already exists`, t.ast.ref);
                }
                t.receivers.push({
                    selector: f.selector,
                    ast: (0, clone_1.cloneNode)(f.ast),
    ...
    function processType(name) {
        ...
        if (processing.has(name)) {
            (0, errors_1.throwCompilationError)(`Circular dependency detected for type "${name}"`, types.get(name).ast.ref);
        }
        processing.has(name);
    ...
    for (const a of ast.functions) {
        ...
        if (r.self) {
            if (types.get(r.self).functions.has(r.name)) {
                (0, errors_1.throwCompilationError)(`Function "${r.name}" already exists in type "${r.self}"`, r.ast.ref);
            }
            types.get(r.self).functions.set(r.name, r);
        }
        else {
            if (staticFunctions.has(r.name) || global_1.GlobalFunctions.has(r.name)) {
                (0, errors_1.throwCompilationError)(`Static function "${r.name}" already exists`, r.ast.ref);
            }
            if (staticConstants.has(r.name)) {
                (0, errors_1.throwCompilationError)(`Static constant "${r.name}" already exists`, a.ref);
            }
    ...
    for (const a of ast.constants) {
        if (staticConstants.has(a.name)) {
            (0, errors_1.throwCompilationError)(`Static constant "${a.name}" already exists`, a.ref);
        }
        if (staticFunctions.has(a.name) || global_1.GlobalFunctions.has(a.name)) {
            (0, errors_1.throwCompilationError)(`Static function "${a.name}" already exists`, a.ref);
        }
...
exports.resolvePartialFields = resolvePartialFields;
